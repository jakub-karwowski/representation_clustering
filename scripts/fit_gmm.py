...
def main():
    ...
    with open(params_path, "r") as f:
        params = yaml.safe_load(f)['wikics']['fit_gmm'][args.config]
    ...
    if params['encoder'] == 'none':
        ...
        embedings_name = f"node_embeddings_gca_{params['encoder']}_{params['dim']}_{params['version']}.pt"
        embedings_path = Path(__file__).resolve().parent.parent / "data" / "embeddings" / embedings_name
    ...
    if params['model'] == 'sklearn':
        model = GaussianMixture(10, max_iter=int(params['max_epochs'], tol=float(params['tol'])), random_state=SEED)
        model.fit(node_embedings.numpy())
        model_file = Path(__file__).resolve().parent.parent / 'models' / 'gmm' / f"gmm_{params['encoder']}_{params['dim']}_{params['version']}.pt"
        with open('gmm_model.pkl', 'wb') as f:
            pickle.dump(model_file, f)
    elif params['model'] == 'diy':
        datamodule = GMMDataModule(node_embedings)
        torch.manual_seed(SEED)
        trainer = get_gmm_trainer('test_gmm', max_epochs=int(params['max_epochs']), termination_threshold=float(params['tol']))
