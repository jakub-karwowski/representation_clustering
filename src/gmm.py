...
class GaussianMixtureModel(pl.LightningModule):
    def __init__(
        ...
        self.means = nn.Parameter(torch.rand((self.n_components, self.dim)), requires_grad=False)
    ...
    def gamma_top(self, x:torch.Tensor) -> torch.Tensor:
        ...
        Sigmas = torch.reshape(cov_reg, (self.n_components, 1, self.dim, self.dim))    
        ...
        p_yx = torch.exp(log_pyx )#- torch.logsumexp(log_pyx, dim=0, keepdim=True)
        return p_yx
    ...
    def maximization_step(
        ...
        n_points = x.shape[0]
        mus = torch.reshape(x, (1, n_points, self.dim, 1)) - torch.reshape(self.means, (self.n_components, 1, self.dim, 1))
        covariances = responsibilities.unsqueeze(-1).unsqueeze(-1) * (mus @ mus.transpose(3, 2))
    ...
    def training_step(self, x: torch.Tensor):
        ...
        self.log('log-likelihood', log_likelihood, on_epoch=True, on_step=False)

    def configure_optimizers(self):
        return None

def get_gmm_trainer(model_name: str, max_epochs: int, patience: int = 1, termination_threshold: float = 1e-3):
